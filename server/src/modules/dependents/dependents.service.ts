...
export class DependentsService {
  ...
  async getDependentById(id: string, actor: CurrentUserDto): Promise<DependentResponseDto> {
    ...
    if (!dependent) {
      throw new NotFoundException({ code: 'NOT_FOUND', message: 'Dependent not found' });
    }
  ...
  async updateDependent(id: string, dto: UpdateDependentDto, actor: CurrentUserDto): Promise<DependentResponseDto> {
  ...
  async listDependents(query: ListDependentsQueryDto, actor: CurrentUserDto): Promise<PaginatedDependentsResponseDto> {
    ...
    if (role === 'corporate') {
      const corporate = await this.prisma.corporate.findUnique({ where: { userId: actor.id }, select: { id: true } });
      if (!corporate) {
        throw new ForbiddenException({ code: 'AUTH_FORBIDDEN', message: 'Corporate profile not found' });
      }
    ...
    if (role === 'patient') {
      const employee = await this.prisma.employee.findUnique({ where: { userId: actor.id }, select: { id: true } });
      if (!employee) {
        throw new ForbiddenException({ code: 'AUTH_FORBIDDEN', message: 'Employee profile not found' });
      }
  ...
  async approveDependent(id: string, actor: CurrentUserDto): Promise<DependentResponseDto> {
    const dependent = await this.prisma.dependent.findUnique({
      where: { id },
    ...
    this.ensureCorporateReviewAccess(dependent.employee.corporate.userId, actor);
  ...
  async rejectDependent(id: string, dto: RejectDependentDto, actor: CurrentUserDto): Promise<DependentResponseDto> {
    const dependent = await this.prisma.dependent.findUnique({
      where: { id },
      include: { employee: { include: { corporate: true } } },
    });
  ...
  async updateDependentStatus(id: string, dto: UpdateDependentStatusDto, actor: CurrentUserDto): Promise<DependentResponseDto> {
    const dependent = await this.prisma.dependent.findUnique({
      where: { id },
      include: { employee: { include: { corporate: true } } },
    });
    ...
    const nextStatus = dto.status === 'Active' ? DependentStatus.Active : DependentStatus.Inactive;
    const updated = await this.prisma.dependent.update({
      where: { id },
  ...
  private ensureCanRequestDependent(
    ...
    throw new ForbiddenException({ code: 'AUTH_FORBIDDEN', message: 'You cannot request dependents for this employee' });
  }

  private ensureCorporateReviewAccess(corporateUserId: string, actor: CurrentUserDto): void {
    const role = actor.role as unknown as string;
    if (role === 'admin') {
      return;
    }
    ...
    throw new ForbiddenException({ code: 'AUTH_FORBIDDEN', message: 'Only owning corporate can review dependent requests' });
  }
  ...
  private ensureDependentAccess(
    ...
    if (role === 'corporate' && dependent.employee.corporate.userId === actor.id) {
      return;
    }
    ...
    if (
      role === 'insurer' &&
      (dependent.status === DependentStatus.Approved || dependent.status === DependentStatus.Active)
    ) {
      return;
    }

    throw new ForbiddenException({ code: 'AUTH_FORBIDDEN', message: 'You are not allowed to view this dependent' });
  }

  private validateRelationshipAge(relationship: string, dateOfBirth: string): void {
    ...
    if (relationship === 'Spouse' && age < 18) {
      throw new BadRequestException({ code: 'VALIDATION_FAILED', message: 'Spouse must be at least 18 years old' });
    }

    if ((relationship === 'Son' || relationship === 'Daughter') && age >= 25) {
      throw new BadRequestException({ code: 'VALIDATION_FAILED', message: 'Child must be under 25 years old' });
    }

    if ((relationship === 'Father' || relationship === 'Mother') && age < 45) {
      throw new BadRequestException({ code: 'VALIDATION_FAILED', message: 'Parent must be at least 45 years old' });
    }
  }
  ...
  private toDependentResponse(dependent: {
    ...
    return {
      ...
      ...(dependent.reviewedDate ? { reviewedDate: dependent.reviewedDate } : {}),
      ...(dependent.rejectionReason ? { rejectionReason: dependent.rejectionReason } : {}),
