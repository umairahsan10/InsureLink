...
function generate() {
  ...
  for (const c of claims) {
    ...
    if (status === "Approved")
      approvedValueTotal += Number(c.approvedAmount || 0);

    const month = formatMonth(
      c.createdAt || c.admissionDate || c.date || new Date().toISOString()
    );
    monthly[month] = monthly[month] || { count: 0, value: 0 };
    monthly[month].count += 1;
    monthly[month].value += amount;

    perCorporate[c.corporateId] = perCorporate[c.corporateId] || {
      corporateId: c.corporateId,
      corporateName: c.corporateName || c.corporateId,
      count: 0,
      value: 0,
    };
    ...
    perHospital[c.hospitalId] = perHospital[c.hospitalId] || {
      hospitalId: c.hospitalId,
      hospitalName: c.hospitalName || c.hospitalId,
      totalAmount: 0,
    };
    perHospital[c.hospitalId].totalAmount += amount;

    if (typeof c.fraudRiskScore === "number" && c.fraudRiskScore > 0.3)
      fraudFlaggedCount++;
  }

  const monthlyTrends = Object.keys(monthly)
    .sort()
    .map((m) => ({
      month: m,
  ...
  const topHospitalsByAmount = Object.values(perHospital)
    .sort((a, b) => b.totalAmount - a.totalAmount)
    .slice(0, 3);
  const rejectionRate =
    totalClaims === 0 ? 0 : (claimsByStatus["Rejected"] || 0) / totalClaims;
