...
export default function HospitalPatientDetailsPage() {
  ...
  const handleSelectPatient = (patient: Employee) => {
    ...
    const patientClaims = claims.filter(claim => claim.employeeId === patient.id);
        ...
        <main className="flex-1 p-4 lg:p-6">
          ...
          {selectedPatient && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">{selectedPatient.employeeNumber}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">{selectedPatient.mobile}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Designation</p>
                    <p className="font-semibold text-gray-900">{selectedPatient.designation}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Department</p>
                    <p className="font-semibold text-gray-900">{selectedPatient.department}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">{formatDate(selectedPatient.coverageStart)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Coverage End</p>
                    <p className="font-semibold text-gray-900">{formatDate(selectedPatient.coverageEnd)}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{formatCurrency(plan.sumInsured)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Deductible</p>
                        <p className="font-semibold text-gray-900">{formatCurrency(plan.deductible)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Copay</p>
                        <p className="font-semibold text-gray-900">{plan.copayPercent}%</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Valid From</p>
                        <p className="font-semibold text-gray-900">{formatDate(plan.validFrom)}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500 mb-2">Covered Services</p>
              ...
              {(() => {
                const corporate = getCorporateDetails(selectedPatient.corporateId);
                  ...
                  <div className="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{corporate.hrContact.name}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Email</p>
                        <p className="font-semibold text-gray-900">{corporate.hrContact.email}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Phone</p>
                        <p className="font-semibold text-gray-900">{corporate.hrContact.phone}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Total Employees</p>
                        <p className="font-semibold text-gray-900">{corporate.totalEmployees}</p>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                {filteredClaims.length === 0 ? (
                  <p className="text-gray-500 text-center py-8">No claims found for this patient.</p>
                ) : (
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Amount Claimed</th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredClaims.map((claim) => (
                          <tr key={claim.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{claim.claimNumber}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{claim.hospitalName}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{formatDate(claim.admissionDate)}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{formatCurrency(claim.amountClaimed)}</td>
          ...
          {!selectedPatient && searchResults.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Search Results ({searchResults.length})</h2>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{patient.email} • {patient.mobile}</p>
                        <p className="text-sm text-gray-500">#{patient.employeeNumber} • {patient.designation}</p>
                      </div>
          ...
          {/* No Results */}
          {!selectedPatient && searchTerm && searchResults.length === 0 && !isSearching && (
            <div className="bg-white rounded-lg shadow-sm p-6 text-center">
              <svg className="w-12 h-12 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-gray-500">No patients found. Try a different search term.</p>
            </div>
          )}
        </main>
      </div>
