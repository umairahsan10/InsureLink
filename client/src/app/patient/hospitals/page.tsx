...
const TAB_STORAGE_KEY = 'insurelink:patient-hospitals-tab';
...
export default function PatientHospitalsPage() {
  ...
  useEffect(() => {
    ...
    if (stored === 'finder' || stored === 'directory') {
      setActiveTab(stored);
    }
  }, []);
  ...
  const karachiHospitals = useMemo(
    () => hospitalsData.filter((hospital) => hospital.city.toLowerCase() === 'karachi'),
    []
  );
