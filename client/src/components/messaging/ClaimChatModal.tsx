...
export default function ClaimChatModal({
  ...
  const handleMessageRead = useCallback(
    (data: { claimId: string; readBy: string }) => {
    ...
    if (filesToUpload.length > 0) {
      ...
      try {
        const results = await Promise.all(
          filesToUpload.map((f) => messagingApi.uploadAttachment(f)),
        );
  ...
  return (
    <div
      className="fixed bottom-0 right-4 w-96 bg-white rounded-t-lg shadow-2xl border border-gray-200 z-50 flex flex-col"
          ...
          messages.map((message) => {
            ...
            return (
              ...
              >
                ...
                >
                  {message.messageText && (
                    <div className="text-sm mb-1">{message.messageText}</div>
                  )}
          ...
          <div className="flex flex-wrap gap-2">
            {pendingFiles.map((file, index) => (
              ...
              >
                ...
                <span className="max-w-[120px] truncate text-gray-900 font-medium">
                  {file.name}
                </span>
                <button
