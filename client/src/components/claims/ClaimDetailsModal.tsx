...
export default function ClaimDetailsModal({ claim, isOpen, onClose, onDecision }: ClaimDetailsModalProps) {
  ...
  const quickStats = [
    { label: 'Turnaround Target', value: '48 hrs', accent: 'from-indigo-500/10 to-blue-500/10' },
    { label: 'Audit Risk', value: 'Low', accent: 'from-emerald-500/10 to-green-500/10' },
    { label: 'Docs Received', value: '4 of 4', accent: 'from-amber-500/10 to-orange-500/10' }
  ];
      ...
      <div className="relative w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl animate-fade-in">
        <div className="relative overflow-hidden border-b border-gray-100 px-8 py-6">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.35em] text-gray-500">Claim Overview</p>
              <div className="mt-1 flex flex-wrap items-center gap-3">
                <h3 className="text-2xl font-semibold text-gray-900">{claim.id}</h3>
          ...
          <div className="relative mt-4 flex flex-wrap items-center gap-3">
            ...
            <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${statusChip}`}>
              {claim.status}
            </span>
        ...
        <div className="max-h-[75vh] overflow-y-auto bg-slate-50 px-8 py-6 space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            {detailRows.map((row) => (
              <div key={row.label} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                <p className="text-[11px] uppercase text-gray-500">{row.label}</p>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {quickStats.map((card) => (
              <div key={card.label} className={`rounded-2xl border border-white/70 bg-gradient-to-br ${card.accent} p-4 shadow-sm`}>
                <p className="text-[11px] uppercase text-gray-600">{card.label}</p>
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-sm text-gray-700">
              This is a simulated claim detail view. Integrate actual narrative, document links, or audit history from
              the backend to enrich this space.
            </p>
          </div>
