...
interface ClaimActionDrawerProps {
  ...
  onClose: () => void;
  onDecision?: (claimId: string, action: 'approve' | 'reject', notes?: string) => void;
  ...
  const infoRows = [
    {
      ...
      icon: (
        <svg className="h-4 w-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A4.002 4.002 0 0112 15a4.002 4.002 0 016.879 2.804M15 11a3 3 0 10-6 0 3 3 0 006 0z" />
        </svg>
      )
    },
    {
      ...
      icon: (
        <svg className="h-4 w-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 22V12m0 0H2l10-9 10 9h-10z" />
        </svg>
      )
    },
    {
      ...
      icon: (
        <svg className="h-4 w-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-2.21 0-4-1.343-4-3s1.79-3 4-3 4 1.343 4 3-1.79 3-4 3zM6 9h12v11H6z" />
        </svg>
      )
    },
    {
      ...
      icon: (
        <svg className="h-4 w-4 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10m-11 8h12a2 2 0 002-2V7H5v12a2 2 0 002 2z" />
        </svg>
      )
    },
    {
      ...
      <div className="relative flex h-full max-h-screen w-full max-w-md flex-col bg-white shadow-2xl animate-slide-in-right">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 bg-gradient-to-r from-slate-50 to-white">
          <div className="space-y-1">
            <p className="text-xs uppercase tracking-[0.2em] text-gray-500">Claim</p>
            <div className="flex items-center gap-3">
              <h3 className="text-xl font-semibold text-gray-900">{claim.id}</h3>
              <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${statusStyles}`}>
                {claim.status}
              </span>
            </div>
            <p className="text-sm text-gray-500">{claim.patient} · {claim.hospital}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 bg-slate-50">
          <div className="rounded-2xl border border-gray-100 bg-white/80 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-semibold text-gray-900">Quick Facts</h4>
              <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium border ${priorityStyles}`}>
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {infoRows.map((row) => (
                <div key={row.label} className="flex items-start gap-2 rounded-xl bg-gray-50/70 px-3 py-2">
                  <div className="mt-0.5">{row.icon}</div>
                  <div className="text-sm">
                    <dt className="text-[12px] uppercase tracking-wide text-gray-500">{row.label}</dt>
          ...
          {mode === 'view' ? (
            <div className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-4 shadow-sm">
              <h4 className="text-sm font-semibold text-blue-900">Claim Summary</h4>
              <p className="mt-2 text-sm text-blue-900/80">
                Review the claim details and status. Switch to review mode from dashboards to provide a decision.
              </p>
            </div>
          ) : (
            <>
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-semibold text-gray-900">Review Notes</label>
                  {hasSavedNotes && (
                    <span className="text-xs font-semibold text-green-600">Notes saved</span>
                  )}
                </div>
                <textarea
                  rows={4}
                ...
                <p className="mt-2 text-xs text-gray-500">
                  Your notes are visible to other reviewers before a final decision is made.
                </p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
