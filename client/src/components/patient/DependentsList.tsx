...
export default function DependentsList({ dependents, onRequestAdd }: DependentsListProps) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-1 text-sm text-gray-600">
        {dependent.status === 'Approved' && (
          <p>
            <span className="font-medium">Coverage:</span> Active since {formatDate(dependent.coverageStartDate)}
          </p>
        )}
        {dependent.status === 'Pending' && (
          <p>
            <span className="font-medium">Requested:</span> {formatDate(dependent.requestedAt)}
          </p>
        )}
        {dependent.status === 'Rejected' && dependent.rejectionReason && (
          <p className="text-red-600">
            <span className="font-medium">Reason:</span> {dependent.rejectionReason}
          </p>
        )}
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold text-gray-900">My Dependents & Family Coverage</h2>
        <button
      ...
      {approved.length === 0 && pending.length === 0 && rejected.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No dependents yet</h3>
          <p className="mt-1 text-sm text-gray-500">Add a family member to your coverage</p>
          <button
            onClick={onRequestAdd}
