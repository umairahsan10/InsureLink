...
interface ClaimsMessagingContextType {
  ...
  sendMessage: (
    ...
    optimisticAttachments?: InlineAttachment[],
  ) => Promise<ClaimMessage | null>;
...
const ClaimsMessagingContext = createContext<
  ClaimsMessagingContextType | undefined
>(undefined);

export function ClaimsMessagingProvider({
  children,
}: {
  children: React.ReactNode;
}) {
  const [messages, setMessages] = useState<Map<string, ClaimMessage[]>>(
    new Map(),
  );
  ...
  const fetchMessages = useCallback(
    async (claimId: string, page = 1, limit = 50) => {
      try {
  ...
  const sendMessage = useCallback(
    async (
      ...
      if (sender) {
        const optimisticAtts: Attachment[] = (optimisticAttachments || []).map(
          (a, i) => ({
            id: `temp-att-${i}`,
            filename: a.filename,
            filePath: a.filePath,
      ...
      try {
        ...
        setMessages((prev) => {
          ...
          const deduped = existing.filter(
            (m) => m.id !== tempId && m.id !== newMessage.id,
          );
  ...
  const addRealtimeMessage = useCallback(
    (
      ...
      const isFromCurrentUser =
        !!options?.currentUserId && message.senderId === options.currentUserId;
...
export function useClaimsMessaging() {
  ...
  if (context === undefined) {
    throw new Error(
      "useClaimsMessaging must be used within a ClaimsMessagingProvider",
    );
  }
  return context;
}
